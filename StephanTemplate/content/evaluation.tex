...
The performance of handling the click events stays constant in our tests no matter the number of gate markers on the map. But we can see that the rendering performance of the floorplan if the user drags or zooms is dependant on the number of gate markers. At an amount of 1000 gate markers in the view frame the frames per second at dragging or scrolling drop to 2 frames per second. This makes the interactive floorplan unresponsive and therefore uncomfortable to use at large amounts of gate markers. Better handling of the map rendering is necessary here.
